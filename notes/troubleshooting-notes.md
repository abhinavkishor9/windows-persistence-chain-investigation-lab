...
## 13. Final Investigation Lesson

The main lesson from this lab is:

```text
Artifact discovery is not the same as execution confirmation.
```

A reliable investigation should progressively answer:

```text
Does the persistence mechanism exist?
        ↓
What does it execute?
        ↓
Does the referenced file exist?
        ↓
Was the file/process executed?
        ↓
Was the expected parent/child relationship observed?
        ↓
Did supporting activity occur?
        ↓
Can the telemetry be correlated?
```

Only after these questions are answered should the analyst reconstruct the persistence chain.
